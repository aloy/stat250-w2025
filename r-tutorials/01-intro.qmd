...
* What does R call a quantitative variable? A categorical variable?
...
* Other useful summary statistics for quantitative variables include (here `x` denotes a column):
    ...
    Calculare the median of `flipper_length_mm` and the variance of `body_mass_g`. Be sure to add `na.rm = TRUE` for each!
...
Here we use the `~` to tell R the `bill_length_mm` is a variable in the data set. We must also specify `data` and you an adjust the number of `bins`.
...
```{webr}
library(janitor)
tabyl(penguins, island) #  Pass in the data frame and then the column name to tabulate
```
